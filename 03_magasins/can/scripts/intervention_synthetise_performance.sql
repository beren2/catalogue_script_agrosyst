SELECT
    intervention_synthetise_id,
    approche_de_calcul,
    C_main_oeuvre_manuelle_reelle,
    C_main_oeuvre_manuelle_reelle_chmps_non_rens,
    C_main_oeuvre_manuelle_reelle_tx_comp,
    C_main_oeuvre_manuelle_std_mil,
    C_main_oeuvre_manuelle_std_mil_chmps_non_rens,
    C_main_oeuvre_manuelle_std_mil_tx_comp,
    C_main_oeuvre_tot_reelle,
    C_main_oeuvre_tot_reelle_chmps_non_rens,
    C_main_oeuvre_tot_reelle_tx_comp,
    C_main_oeuvre_tot_std_mil,
    C_main_oeuvre_tot_std_mil_chmps_non_rens,
    C_main_oeuvre_tot_std_mil_tx_comp,
    C_main_oeuvre_tractoriste_reelle,
    C_main_oeuvre_tractoriste_reelle_chmps_non_rens,
    C_main_oeuvre_tractoriste_reelle_tx_comp,
    C_main_oeuvre_tractoriste_std_mil,
    C_main_oeuvre_tractoriste_std_mil_chmps_non_rens,
    C_main_oeuvre_tractoriste_std_mil_tx_comp,
    cm_reel_chmps_non_rens,
    cm_reelles,
    cm_reelles_tx_comp,
    cm_std_mil,
    cm_std_mil_chmps_non_rens,
    cm_std_mil_tx_comp,
    co_decomposees_reel_chp_non_rens,
    co_decomposees_reel_tx_comp,
    co_decomposees_std_mil_chp_non_rens,
    co_decomposees_std_mil_tx_comp,
    co_epandage_orga_reelles,
    co_epandage_orga_std_mil,
    co_fertimin_reel,
    co_fertimin_std_mil,
    co_intrants_autres_reelles,
    co_intrants_autres_std_mil,
    co_irrigation_reelles,
    co_irrigation_std_mil,
    conso_carburant,
    conso_carburant_chmps_non_rens,
    conso_carburant_tx_comp,
    conso_eau,
    conso_eau_chmps_non_rens,
    conso_eau_tx_comp,
    co_phyto_avec_amm_reelles,
    co_phyto_avec_amm_std_mil,
    co_phyto_sans_amm_reelles,
    co_phyto_sans_amm_std_mil,
    co_pots_reelles,
    co_pots_std_mil,
    co_semis_reel,
    co_semis_std_mil,
    co_substrats_reelles,
    co_substrats_std_mil,
    co_tot_reell_chmps_non_rens,
    co_tot_reelles,
    co_tot_reelles_tx_comp,
    co_tot_std_mil,
    co_tot_std_mil_chp_non_rens,
    co_tot_std_mil_tx_comp,
    co_trait_semence_reelles,
    co_trait_semence_std_mil,
    ferti_B_mineral,
    ferti_Ca_mineral,
    ferti_CaO_organique,
    ferti_Cu_mineral,
    ferti_Fe_mineral,
    ferti_K2O_mineral,
    ferti_K2O_organique,
    ferti_K2O_tot,
    ferti_MgO_mineral,
    ferti_MgO_organique,
    ferti_Mn_mineral,
    ferti_Mo_mineral,
    ferti_Na2O_mineral,
    ferti_N_mineral,
    ferti_N_organique,
    ferti_N_tot,
    ferti_P2O5_mineral,
    ferti_P2O5_organique,
    ferti_P2O5_tot,
    ferti_SO3_mineral,
    ferti_S_organique,
    ferti_Zn_mineral,
    hri1_g1_hts,
    hri1_g1_tot,
    hri1_g1_tot_detail_champs_non_renseig,
    hri1_g1_tot_taux_de_completion,
    hri1_g2_hts,
    hri1_g2_tot,
    hri1_g2_tot_detail_champs_non_renseig,
    hri1_g2_tot_taux_de_completion,
    hri1_g3_hts,
    hri1_g3_tot,
    hri1_g3_tot_detail_champs_non_renseig,
    hri1_g3_tot_taux_de_completion,
    hri1_g4_hts,
    hri1_g4_tot,
    hri1_g4_tot_detail_champs_non_renseig,
    hri1_g4_tot_taux_de_completion,
    hri1_hts,
    hri1_tot,
    hri1_tot_detail_champs_non_renseig,
    hri1_tot_taux_de_completion,
    ift_cible_mil_a,
    ift_cible_mil_biocontrole,
    ift_cible_mil_chimique_tot,
    ift_cible_mil_chim_tot_hts,
    ift_cible_mil_chmps_non_rens,
    ift_cible_mil_f,
    ift_cible_mil_h,
    ift_cible_mil_hh,
    ift_cible_mil_i,
    ift_cible_mil_ts,
    ift_cible_mil_tx_comp,
    ift_cible_non_mil_a,
    ift_cible_non_mil_biocontrole,
    ift_cible_non_mil_chimique_tot,
    ift_cible_non_mil_chim_tot_hts,
    ift_cible_non_mil_chmps_non_rens,
    ift_cible_non_mil_f,
    ift_cible_non_mil_h,
    ift_cible_non_mil_hh,
    ift_cible_non_mil_i,
    ift_cible_non_mil_ts,
    ift_cible_non_mil_tx_comp,
    ift_culture_mil_a,
    ift_culture_mil_biocontrole,
    ift_culture_mil_chimique_tot,
    ift_culture_mil_chim_tot_hts,
    ift_culture_mil_chmps_non_rens,
    ift_culture_mil_f,
    ift_culture_mil_h,
    ift_culture_mil_hh,
    ift_culture_mil_i,
    ift_culture_mil_ts,
    ift_culture_mil_tx_comp,
    ift_culture_non_mil_a,
    ift_culture_non_mil_biocontrole,
    ift_culture_non_mil_chimique_tot,
    ift_culture_non_mil_chim_tot_hts,
    ift_culture_non_mil_chmp_no_rens,
    ift_culture_non_mil_f,
    ift_culture_non_mil_h,
    ift_culture_non_mil_hh,
    ift_culture_non_mil_i,
    ift_culture_non_mil_ts,
    ift_culture_non_mil_tx_comp,
    ift_histo_a,
    ift_histo_biocontrole,
    ift_histo_chimique_tot,
    ift_histo_chim_tot_hts,
    ift_histo_chmps_non_rens,
    ift_histo_f,
    ift_histo_h,
    ift_histo_hh,
    ift_histo_i,
    ift_histo_ts,
    ift_histo_tx_comp,
    intervention_synthetise_id,
    nbre_de_passages,
    nbre_de_passages_aout,
    nbre_de_passages_avril,
    nbre_de_passages_chmps_non_rens,
    nbre_de_passages_decembre,
    nbre_de_passages_desherbage_meca,
    nbre_de_passages_desherbage_meca_chmps_non_rens,
    nbre_de_passages_desherbage_meca_tx_comp,
    nbre_de_passages_fevrier,
    nbre_de_passages_janvier,
    nbre_de_passages_juillet,
    nbre_de_passages_juin,
    nbre_de_passages_labour,
    nbre_de_passages_labour_chmps_non_rens,
    nbre_de_passages_labour_tx_comp,
    nbre_de_passages_mai,
    nbre_de_passages_mars,
    nbre_de_passages_novembre,
    nbre_de_passages_octobre,
    nbre_de_passages_septembre,
    nbre_de_passages_tcs,
    nbre_de_passages_tcs_chmps_non_rens,
    nbre_de_passages_tcs_tx_comp,
    nbre_de_passages_tx_comp,
    qsa_boscalid,
    qsa_boscalid_hts,
    qsa_chlortoluron,
    qsa_chlortoluron_hts,
    qsa_cmr,
    qsa_cmr_hts,
    qsa_cuivre_ferti,
    qsa_cuivre_phyto,
    qsa_cuivre_phyto_hts,
    qsa_cuivre_tot,
    qsa_cuivre_tot_hts,
    qsa_danger_environnement,
    qsa_danger_environnement_hts,
    qsa_diflufenican,
    qsa_diflufenican_hts,
    qsa_fluopyram,
    qsa_fluopyram_hts,
    qsa_glyphosate,
    qsa_glyphosate_hts,
    qsa_lambda_cyhalothrine,
    qsa_lambda_cyhalothrine_hts,
    qsa_prosulfocarbe,
    qsa_prosulfocarbe_hts,
    qsa_smetolachlore,
    qsa_smetolachlore_hts,
    qsa_soufre_ferti,
    qsa_soufre_phyto,
    qsa_soufre_phyto_hts,
    qsa_soufre_tot,
    qsa_soufre_tot_hts,
    qsa_substances_candidates_substitution,
    qsa_substances_candidates_substitution_hts,
    qsa_substances_faible_risque,
    qsa_substances_faible_risque_hts,
    qsa_tot,
    qsa_tot_detail_champs_non_renseig,
    qsa_tot_hts,
    qsa_tot_taux_de_completion,
    qsa_toxique_utilisateur,
    qsa_toxique_utilisateur_hts,
    recours_aux_moyens_biologiques,
    recours_macroorganismes,
    recours_produits_abiotiques_sansamm,
    recours_produits_biotiques_sansamm,
    tps_travail_manuel,
    tps_travail_manuel_aout,
    tps_travail_manuel_avril,
    tps_travail_manuel_decembre,
    tps_travail_manuel_fevrier,
    tps_travail_manuel_janvier,
    tps_travail_manuel_juillet,
    tps_travail_manuel_juin,
    tps_travail_manuel_mai,
    tps_travail_manuel_mars,
    tps_travail_manuel_novembre,
    tps_travail_manuel_octobre,
    tps_travail_manuel_septembre,
    tps_travail_manuel_tx_comp,
    tps_travail_mecanise,
    tps_travail_mecanise_aout,
    tps_travail_mecanise_avril,
    tps_travail_mecanise_decembre,
    tps_travail_mecanise_fevrier,
    tps_travail_mecanise_janvier,
    tps_travail_mecanise_juillet,
    tps_travail_mecanise_juin,
    tps_travail_mecanise_mai,
    tps_travail_mecanise_mars,
    tps_travail_mecanise_novembre,
    tps_travail_mecanise_octobre,
    tps_travail_mecanise_septembre,
    tps_travail_mecanise_tx_comp,
    tps_travail_total,
    tps_travail_total_aout,
    tps_travail_total_avril,
    tps_travail_total_decembre,
    tps_travail_total_fevrier,
    tps_travail_total_janvier,
    tps_travail_total_juillet,
    tps_travail_total_juin,
    tps_travail_total_mai,
    tps_travail_total_mars,
    tps_travail_total_novembre,
    tps_travail_total_octobre,
    tps_travail_total_septembre,
    tps_travail_total_tx_comp,
    tps_trav_manuel_chmps_non_rens,
    tps_trav_mecanise_chmps_non_rens,
    tps_trav_total_chmps_non_rens,
    tps_utilisation_materiel,
    tps_utilisation_materiel_aout,
    tps_utilisation_materiel_avril,
    tps_utilisation_materiel_dec,
    tps_utilisation_materiel_fevrier,
    tps_utilisation_materiel_janvier,
    tps_utilisation_materiel_juillet,
    tps_utilisation_materiel_juin,
    tps_utilisation_materiel_mai,
    tps_utilisation_materiel_mars,
    tps_utilisation_materiel_nov,
    tps_utilisation_materiel_oct,
    tps_utilisation_materiel_sept,
    tps_utilisation_materiel_tx_comp,
    tps_util_materiel_chmps_non_rens,
    type_de_travail_du_sol,
    type_de_travail_du_sol_chmps_non_rens,
    type_de_travail_du_sol_tx_comp,
    utili_desherbage_meca,
    utili_desherbage_meca_chmps_non_rens,
    utili_desherbage_meca_tx_comp
FROM entrepot_intervention_synthetise_performance;